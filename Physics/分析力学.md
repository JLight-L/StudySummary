...
# 对力学系统的描述
用参量 $\mathbf{r}$ 描述系统状态
**经典**框架： $\mathbf{r}$ 包括空间位置信息，$\mathbf{r}$ 为关于时间的函数

## 描述位置
**def**  自由度 $s$ ：描述系统位形所需的最小的参数个数
**def**  约束：对系统参数的限制，数学上表现为对某种函数值的额外要求（即方程）
...
### 哈密顿方程
$$
\begin{cases} \frac{d}{dt}p=-\frac{ \partial  }{ \partial q }H \\ \frac{d}{dt}q=\frac{ \partial  }{ \partial p }H\end{cases}
$$
> 哈密顿方程是关于一对变量（通常被称为“共轭变量”）的方程，……
